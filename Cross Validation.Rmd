...
Split this data into training and test sets (using anti_join!!), 
and replot showing the split. 
...
Applied the wiggly model that account for the noise (bounce up and down), making slightly worse predictions -- Overfitting. 
```{r}
train_df |> 
...
`crossv_mc` saves the data once and stores the indexes for each training / testing split using a `resample` object, and can often be treated exactly like a dataframe. 
However, it’s not compatible with `gam`, so we have to convert each training and testing dataset (and lose that nice memory-saving stuff in the process) using the code below. 
*It’s worth noting, though, that if all the models you want to fit use `lm`, you can skip this.
```{r}
...
child_growth |> 
  ...
  facet_grid(~model)
```
It’s not clear which is best – the linear model is maybe too simple, but the piecewise and non-linear models are pretty similar! 
